...
class AvgustPravaPlataSeeder extends Seeder
{
...
//        Log::channel('user_action')->debug('Test custom logger START MART');
        ...
        foreach ($newDataPodaciMesec as $data){
            if ($data['M_G'] == '0324') {
                ...
                $date = Carbon::createFromFormat('my', '0824')->startOfMonth()->setDay(1);
...
//        }
    ...
    {
        $filePath = storage_path('app/backup/avgustPlataNovo/DKOE.csv');
    ...
    {
        $filePath = storage_path('app/backup/avgustPlataNovo/DFVP.csv');
        $csv = Reader::createFromPath($filePath, 'r');
        $csv->setHeaderOffset(0);
        $csv->setDelimiter(',');
        return $csv->getRecords();
    ...
    {
        $filePath = storage_path('app/backup/avgustPlataNovo/MKRE.csv');
        $csv = Reader::createFromPath($filePath, 'r');
        $csv->setHeaderOffset(0);
        $csv->setDelimiter(',');
        return $csv->getRecords();
